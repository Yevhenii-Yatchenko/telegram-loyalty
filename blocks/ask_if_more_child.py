...
class AskIfMoreChild(AnswerRequestor):
    request_message = localz.request_more_child
    error_message = localz.incorrect_yes_no
    add_markup = reply_markup
    remove_markup = remove_markup
    block_id = 'ask_if_more_child'
    ...
    def process_message(self, message: Message):
        ...
        if message.text == model_no:
            api().reply_to(message, localz.more_child_canceled, reply_markup = self.remove_markup)
